...
loaded_model.load_weights("generator_weight.hdf5")
...
for i in range(generate):
    ...
    sampled_labels = np.zeros(generate)  # dummy sample label
    gen_imgs = loaded_model.predict([noise, sampled_labels])
    gen_imgs = gen_imgs[0,:,:,:]
    #Rescale images 0 - 1
    gen_imgs= 0.5 * gen_imgs + 0.5
    #gen_imgs_up = resize(gen_imgs, (128, 128))   #resize sample to 128x128 
...
np.save('X_gen_' , X_gen)
np.save('y_gen_' , y_gen)
